...
The Rust version reads and inserts data batch by batch through the async `mysql_async` driver. The default batch size is `1000`, the default row limit is `100000`, and an aggregated import/freshness progress summary is printed every `3` seconds by default. When a batch insert fails, it retries up to `10` times with a `1` second interval.
...
The Rust version adds `--conn-pool-size`, default `1000`. A single shared async MySQL connection pool is created for the whole import run, and each batch insert / freshness query acquires one connection from that pool immediately before execution and returns it right after the current SQL finishes.
In the Rust version, multi-table import scheduling and per-table database work are both driven by Tokio async tasks. Retry waiting and freshness polling also use non-blocking `tokio::time::sleep`.
For each freshness window, the Rust version parses the CSV file only once and shares the parsed batch list across all target-table tasks in that window, so increasing `--count` does not multiply CSV parsing work.
For execute mode, the Rust version also prebuilds the parameterized batch payload once per freshness window, executes each batch as one multi-value `INSERT ... VALUES (...), (...), ...`, and reuses one connection per table task across successful batch inserts, reconnecting only when a batch fails and enters retry.
When multiple target tables are imported concurrently, the Rust version prints one periodic aggregate progress line for the whole freshness window instead of printing one progress line per table. The same aggregation style is also used for concurrent freshness waiting after the import phase switches to visibility polling.
The Rust version prefixes stdout and stderr log lines with a wall-clock timestamp, while keeping raw SQL and raw query result output unchanged.
...
Each `completed import` line is appended to `log/insert_result.log` under the current working directory without being printed to stdout. Insert retry and fatal insert failure messages are also appended to `log/insert_error.log`.
